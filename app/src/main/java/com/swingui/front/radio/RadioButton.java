...
public class RadioButton
{
    ...
    {
        RadioButtonWT<T> radio = new RadioButtonWT<>(new UIValue<>(item));
        return radio;
    }
     ...
     * 
     * @apiNote ラジオ・ボタン横に表示するテキストは、{@code label} で指定された関数の戻り値
     * 
     * @param <T> 選択対象データの型
     * @param item 選択対象データ
     * @param label ラベル生成関数
     * @return 生成された {@code RadioButtonWT} インスタンス
     */
    public static <T> RadioButtonWT<T> of(UIValue<T> item, Function<T, String> label)
    {
        RadioButtonWT<T> radio = new RadioButtonWT<>(item, label);
        return radio;
    }
}
